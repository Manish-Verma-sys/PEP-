...
// Serve all frontend files from the repository root.
...
app.get('/api/alerts', async (req, res) => {
  ...
  if (reg && reg !== 'all') {
    out = out.filter(a => a.reg === reg);
  }

  if (sev && sev !== 'all') {
    out = out.filter(a => a.sev === sev);
  }

  if (focus && focus !== 'all') {
    out = out.filter(a => a.focus === focus);
  }
...
app.get('/api/overview', async (req, res) => {
  ...
  const by = severity =>
    list.filter(a => a.sev === severity).length;

  const skusExposed = list.reduce(
    (n, a) => n + (a.skus ? a.skus.length : 0),
    0
  );
  ...
  res.json({
    ...
    top: list
      .slice(0, 5)
      .map(({ id, sev, reg, when, title, focus, region }) => ({
        id,
        sev,
        reg,
        when,
...
// index.html is in the repository root, not in a public/ folder.
app.get('*', (req, res) => {
...
app.listen(PORT, () => {
  ...
  console.log(
    `provider=${st.provider} key=${st.keyConfigured ? 'set' : 'not set'} live=${liveEnabled()}`
  );
});
